...
func (h *ChunkUploadHandler) UploadChunk(c *gin.Context) {
	...
	//MD5 可以简单理解为：把一段任意长度的数据，计算成一个固定长度的“内容指纹”。
	hash := md5.New()
	//已经把 chunkFile 读到末尾了
	...
	//%x 的意思是：把字节转成小写十六进制字符串
	actualHash := fmt.Sprintf("%x", hash.Sum(nil))
	//确保这个分片在传输过程中没有损坏，也没有传错。
	...
	//意思是：把文件读取指针重新移动到文件开头，这样后面才能再读一遍，把内容写入磁盘
...
func (h *ChunkUploadHandler) CompleteChunkUpload(c *gin.Context) {
	...
	for i := 0; i < session.TotalChunks; i++ {
		...
		//如果某个分片缺失，或者合并失败，就把已经生成了一半的最终文件删掉
		if err != nil {
			finalFile.Close()
			os.Remove(finalPath)
